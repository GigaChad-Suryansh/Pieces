...
document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>tab(b.dataset.tab));document.querySelector('#search').oninput=e=>{state.search=e.target.value;renderLibrary()};document.addEventListener('keydown',e=>{if(e.key.toLowerCase()==='k'){e.preventDefault();document.querySelector('#search').focus()}});
renderLibrary();